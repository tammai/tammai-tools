...
- **Output is a single `.html` file** — no server, no JS bundler, no external runtime dependencies at view-time (fonts load from Google Fonts CDN when online). The deck keeps that CDN `<link>` on purpose and `build.py` has no font-embedding mode; **font self-hosting lives in `slides-to-pdf` instead**, because only the PDF bakes in whatever rendered. See that section below.
...
**Browser discovery has a fallback chain.** `playwright install chromium` pulls from `playwright.azureedge.net`, which some networks block — and the PyPI packages are unaffected, so only the binary needs another source. `find_browser()` resolves, in order: `--browser-path`, `$SLIDES_TO_PDF_BROWSER`, `$PLAYWRIGHT_CHROMIUM_PATH`, `~/.cache/puppeteer` (`npx puppeteer browsers install chrome-headless-shell` reaches googlechromelabs instead), then the usual Chrome/Chromium/Edge install paths. Verified end to end on this machine, with **no `playwright install chromium`**: it resolved the macOS `.app`-nested `Chromium 111.0.5555.0` out of the Puppeteer cache and exported a 15-page PDF at 960 × 540 pt with bookmarks, extractable text and the dark theme intact. A venv at `~/.slides-to-pdf-venv` (the path `slides-to-pdf/SKILL.md` documents) already has the two packages.

**Fonts are embedded at export time, and only here.** The deck keeps its CDN `<link>` (small file, shared cache, correct for an interactive page). A PDF cannot work that way — it is a snapshot, so a blocked CDN is baked in permanently and silently, which is exactly what Claude Cowork produces since it blocks `fonts.googleapis.com`. So `render_slides` reads the families off the live page (`--brand-font-*`, so presets are covered), fetches the woff2 from npm's `@fontsource*` packages — npm stays reachable where Google's CDN does not — and injects them as `data:` URI `@font-face` rules via `add_style_tag`, leaving the deck untouched like every other override. On by default; `--no-embed-fonts` restores the old behaviour.

Measured with the font CDN made unresolvable, page 1 against the online render: embedded differs by **0.19%** of pixels, `--no-embed-fonts` by **2.64%**. Two traps that produce a silently wrong result, both handled in `_faces`:

- **The variable packages name the family `'<Family> Variable'`.** Inject their CSS verbatim and `--brand-font-main: 'Google Sans'` matches nothing — the page falls back while looking correctly wired. The family is forced back to the requested name.
- **They ship `format('woff2-variations')`**, long deprecated; an engine that does not recognise the format string skips that `src` entirely. Normalised to `format('woff2')`, which is correct for variable fonts too.

Two more things that look like over-engineering and are not: the subset match is tried longest-first so `latin-ext` is not swallowed by `latin`, and a variable package's `wght.css`/`standard.css`/`full.css` are *alternatives* covering the same glyphs by a different axis — taking more than one embeds the family twice and doubles the output. Static packages are the opposite: every weight file is a distinct face and all are wanted. Default subsets are `latin,latin-ext,vietnamese`; dropping `vietnamese` for a Vietnamese deck loses the diacritics *only*, so the slide renders in two typefaces at once — harder to notice than a wholesale fallback.

**Don't diagnose a webfont failure from the PDF's font names.** `pypdf` reports page-1 `/BaseFont` values like `/AAAAAA+Helvetica` and `/Courier` for a deck whose fonts loaded perfectly — print-to-PDF labels embedded subsets generically, and Google Sans comes through as `Type3 /None` (vector glyph procedures) rather than a named font program. Two checks tell the truth instead: `[...document.fonts].filter(f => f.status === 'loaded')` in the headless page, and rendering the PDF to an image with `qlmanage -t -s 1400 -o thumbs deck.pdf` and looking at it. Both were run; the letterforms are the requested families.
...
- Content outside a `viewBox` is silently invisible — verify with `getBBox()` against `viewBox.baseVal`. **This is now checked mechanically in two places**, because the documented DevTools snippet caught nothing: a deck shipped on 2026-07-25 with a seven-card stack running to `y=308` inside `viewBox="0 0 460 306"`, so one card lost its bottom border and rounded corners while six kept theirs — invisible in the browser, invisible in the PDF, and invisible to the slide-overflow and column-overlap checks, since neither the slide nor the column was violated. `build.py`'s `check_svg_viewbox()` warns from attributes alone at assembly time; `slides_to_pdf.py` measures `getBBox()` per `<svg>` at export and reports `SVG OUTSIDE VIEWBOX`.
