...
- `scaleDeck()` guards against a zero/NaN viewport — without it, a hidden iframe or snapshot renderer computes `scale(0)` and the deck renders blank. Don't remove the guard.
